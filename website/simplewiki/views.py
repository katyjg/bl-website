...
def view(request, wiki_url):
    ...
    if perm_err:
        return perm_err
    c = {'wiki_article': article,
         'wiki_write': article.can_write_l(request.user),
         'wiki_attachments_write': article.can_attach(request.user),
        }
    return render(request, 'simplewiki_view.html', c)
...
def create(request, wiki_url):
    ...
    if url_path != [] and url_path[0].startswith('_'):
            c = {'wiki_err_keyword': True,
                 'wiki_url': '/'.join(url_path) }
            return render(request, 'simplewiki_error.html', c)
    ...
    return render(request, 'simplewiki_create.html', c)
...
def edit(request, wiki_url):
    ...
    return render(request, 'simplewiki_edit.html', c)
...
def history(request, wiki_url, page=1):
    ...
    return render(request, 'simplewiki_history.html', c)
...
def search_articles(request, wiki_url):
    ...
    if request.method == 'POST':
        ...
        if results.count() == 1:
            ...
            c = {'wiki_search_results': results,
                 'wiki_search_query': querystring
                }
            return render(request, 'simplewiki_searchresults.html', c)
...
def encode_err(request, url):
    return render(request, 'simplewiki_error.html',
                              {'wiki_err_encode': True})
    
def not_found(request, wiki_url):
    """Generate a NOT FOUND message for some URL"""
    return render(request, 'simplewiki_error.html',
                  {'wiki_err_notfound': True,
                   'wiki_url': wiki_url})
