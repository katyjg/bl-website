...
def edit_visit(request, pk, model, form, template='wp-root.html'):
    ...
    if request.method == 'POST':
        ...
        frm = form(instance=visit, initial=dict(list(request.GET.items()))) # casting to a dict pulls out first list item in each value list
        return render(request, template, {
        'info': form_info, 
        'form' : frm,
        })
...
def delete_object(request, pk, model, form, template='wp-root.html'):
    ...
    if request.method == 'POST':
        ...
        frm = form(instance=obj, initial=dict(list(request.GET.items()))) # casting to a dict pulls out first list item in each value list
        return render(request, template, {
            'info': form_info, 
            'form' : frm,
            })
...
def add_object(request, model, form, template='wp-root.html'):
    ...
    if request.method == 'POST':
        ...
        frm = form(initial=list(request.GET.items()))
        if model in [Visit, Stat]:
...
def current_week(request, day=None, template='scheduler/schedule_week.html', admin=False, staff=False, title=''):
    ...
    for day in this_wk:
        ...
        for blkey, blvis in list(bl_week.items()):
            shifts[blkey] = []
